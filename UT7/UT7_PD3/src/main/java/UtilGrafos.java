...
public class UtilGrafos {
    ...
    public static void imprimirMatrizDeWarshallMejorado(boolean[][] matriz, Map<Comparable, TVertice> vertices, String titulo) {
        if (vertices != null && matriz.length == vertices.keySet().size()) {
            ...
            for (int i = 0; i < matriz.length; i++) {
                System.out.print(etiquetas[i]);
                for (int j = 0; j < matriz.length; j++) {
                    Boolean result = matriz[i][j];
                    System.out.print(rellenar(result.toString(), largo, ' '));
                }
                System.out.println();
                System.out.println(linea);
            }
